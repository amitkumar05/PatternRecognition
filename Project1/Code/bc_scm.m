%function for classifying using bayes classifier for same covariance matrix
%calculated by finding covariance matrix of training data of '3' individual classes combined
function [  ] = bc_scm( pathname )
    
    profile on;
    
    
    file = '\ClassComb_train.txt';
    ...
    filename = [pathname '\probT1_bc_scm.txt'];
    writemat(filename, probT1);
    filename = [pathname '\probT2_bc_scm.txt'];
    writemat(filename, probT2);
    filename = [pathname '\probT3_bc_scm.txt'];
    ...
    t='\bf Bayes Classifier with same covariance matrices(classes combined)';
    l1='Class 1';
    l2='Class 2';
    l3='Class 3';
...
%    axis ([-30 30 -30 30]);
    ...
    print([pathname '\bc_scm_allclass'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\bc_scm_class12'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\bc_scm_class23'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\bc_scm_class13'], '-dpng', '-r0');
    
    
    
    %%%Confusion Matrix%%%
    con_mat = con_mat_gen( probT1, probT2, probT3 );
    filename = [pathname '\con_mat_bc_scm.txt'];
