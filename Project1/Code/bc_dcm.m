...
function [] = bc_dcm( pathname )
    ...
    probT1C3 = posterior(testmat, mu3, covmat3);
    ...
    probT2C3 = posterior(testmat, mu3, covmat3);
    ...
    testmat = readmat( filename );
    probT3C1 = posterior(testmat, mu1, covmat1);
    probT3C2 = posterior(testmat, mu2, covmat2);
    probT3C3 = posterior(testmat, mu3, covmat3);
    ...
    filename = [pathname '\probT1_bc_dcm.txt'];
    writemat(filename, probT1);
    filename = [pathname '\probT2_bc_dcm.txt'];
    writemat(filename, probT2);
    filename = [pathname '\probT3_bc_dcm.txt'];
    ...
    t='\bf Bayes Classifier with different covariance matrices';
    l1='Class 1';
    l2='Class 2';
    l3='Class 3';
...
%    pixel = pixel_gen(-30, 30, 0.1, -30, 30, 0.1);
    ...
    p3 = posterior(pixel, mu3, covmat3);
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
     print([pathname '\bc_dcm_allclass'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
     print([pathname '\bc_dcm_class12'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\bc_dcm_class23'], '-dpng', '-r0');
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\bc_dcm_class13'], '-dpng', '-r0');
    
    
    %%%Confusion Matrix%%%
    con_mat = con_mat_gen( probT1, probT2, probT3 );
    filename = [pathname '\con_mat_bc_dcm.txt'];
