...
function [] = nbc_scm_avg( pathname )
    ...
    size_v = size(vmean);
    num_of_ele = size_v(1);
    sig_vmean = sum(vmean) / num_of_ele;
    %vf = zeroes(num_of_ele, 1);
    for k = 1:num_of_ele
        vf(k) = sig_vmean;
    end
    covmat = diag(vf);
    ...
    filename = [pathname '\probT1_nbc_scm_avg.txt'];
    writemat(filename, probT1);
    filename = [pathname '\probT2_nbc_scm_avg.txt'];
    writemat(filename, probT2);
    filename = [pathname '\probT3_nbc_scm_avg.txt'];
    ...
    t='\bf Naive Bayes Classifier with same covariance matrices(\sigma^2I)';
    l1='Class 1';
    l2='Class 2';
    l3='Class 3';
...
%    axis ([-30 30 -30 30]);
    axis ([-500 2000 -500 2000]);
    print([pathname '\nbc_scm_avg_class13'], '-dpng', '-r0');
    
    %%%Confusion Matrix%%%
    con_mat = con_mat_gen( probT1, probT2, probT3 );
    filename = [pathname '\con_mat_nbc_scm_avg.txt'];
